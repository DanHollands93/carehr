...
const StaffShiftCard = ({ 
  ...
  const handleClockIn = () => {
    if (isAdHoc) { proceedWithAction('clock_in'); return; }
  ...
  const handleClockOut = () => {
    if (isAdHoc) { proceedWithAction('clock_out'); return; }
  ...
  const handleWarningConfirm = () => {
    ...
    if (pendingAction) proceedWithAction(pendingAction.type);
  };
      ...
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex space-x-2 pt-2">
            {canClockIn && (
              <Button onClick={handleClockIn} disabled={isClockingIn} className="flex-1" variant="default">
                {isClockingIn ? 'Clocking In...' : 'Clock In'}
              </Button>
            )}
            {canClockOut && (
              <Button onClick={handleClockOut} disabled={isClockingOut} className="flex-1" variant="secondary">
